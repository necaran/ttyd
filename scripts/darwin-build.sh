...
build_libwebsockets() {
    ...
    pushd "${BUILD_DIR}/libwebsockets-${LIBWEBSOCKETS_VERSION}"
        ...
        cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 \
            ...
            -DLIBUV_INCLUDE_DIRS="${STAGE_DIR}/include" -DLIBUV_LIBRARIES="${STAGE_DIR}/lib/libuv.a" \
            ..
        make -j"$(sysctl -n hw.ncpu)" install
    popd
}
...
build_ttyd() {
    ...
    cmake \
        -DCMAKE_FIND_LIBRARY_SUFFIXES=".a" \
        -DCMAKE_EXE_LINKER_FLAGS="-flto -dead_strip" \
        -DCMAKE_BUILD_TYPE=RELEASE \
        -DCMAKE_PREFIX_PATH="${STAGE_DIR}" \
        ..
        make
...
build() {
    ...
    export CFLAGS='-D_FORTIFY_SOURCE=3 -ftrivial-auto-var-init=zero -fstack-protector-strong -O2 -flto -fomit-frame-pointer -ffunction-sections -fdata-sections -fno-unwind-tables -fno-asynchronous-unwind-tables -fvisibility=hidden'
