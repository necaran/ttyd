...
build_ttyd() {
    ...
    cmake -DCMAKE_TOOLCHAIN_FILE="${BUILD_DIR}/cross-${TARGET}.cmake" \
        -DCMAKE_FIND_LIBRARY_SUFFIXES=".a" \
        -DCMAKE_EXE_LINKER_FLAGS="-flto=auto -static -no-pie -s -Wl,-Bsymbolic,--gc-sections" \
        -DCMAKE_BUILD_TYPE=RELEASE \
        ..
    make
}
...
build() {
    ...
    export CFLAGS="-flto -D_FORTIFY_SOURCE=3 -fstack-protector-strong -fstack-clash-protection -O2 -fomit-frame-pointer -ffunction-sections -fdata-sections -fno-unwind-tables -fno-asynchronous-unwind-tables -fvisibility=hidden"
